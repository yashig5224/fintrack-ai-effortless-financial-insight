...
function pageCover(doc: Doc, input: ReportInput, score: number, grade: string) {
  ...
  const metas: [string, string][] = [
    ["PREPARED FOR", input.userName || "TrackMint User"],
  ...
  doc.setFont("helvetica", "normal");
  resetText(doc);
  doc.text("Confidential · Generated by Lumo AI · TrackMint.ai", W(doc) / 2, H(doc) - 28, { align: "center" });
}
...
export async function exportReport(input: ReportInput): Promise<void> {
  ...
  const filename = `TrackMint-${input.kind}-${new Date().toISOString().slice(0, 10)}.pdf`;
  doc.save(filename);
}
