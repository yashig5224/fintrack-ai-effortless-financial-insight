...
const CAT_HEX: Record<string, RGB> = {
  ...
  Other: [148, 163, 184], Transport: [251, 113, 133], Rent: [120, 113, 198],
  Groceries: [234, 88, 12],
};
...
function panel(d: Doc, x: number, y: number, w: number, h: number, title?: string) {
  d.setFillColor(...PAPER); d.roundedRect(x, y, w, h, 8, 8, "F");
  d.setDrawColor(...LINE); d.setLineWidth(0.4); d.roundedRect(x, y, w, h, 8, 8, "S");
  if (title) {
    d.setFontSize(8); d.setFont("helvetica", "bold"); d.setTextColor(...MUTED);
    d.text(title.toUpperCase(), x + 12, y + 14);
  }
}

function brandedHeader(d: Doc, title: string, subtitle: string, tier: string) {
  gradFill(d, 0, 0, W(d), 56, [DEEP, PRIMARY, ACCENT, PINK]);
  // logo mark
  d.setFillColor(...PAPER); d.roundedRect(20, 16, 24, 24, 6, 6, "F");
  d.setTextColor(...PRIMARY); d.setFontSize(13); d.setFont("helvetica", "bold");
  d.text("FT", 32, 31, { align: "center" });
  d.setTextColor(...PAPER); d.setFontSize(13); d.setFont("helvetica", "bold");
  ...
  d.setFillColor(255, 255, 255); d.setGState(new (d as never as { GState: new (o: { opacity: number }) => unknown }).GState({ opacity: 0.18 }));
  d.roundedRect(W(d) - pw - 20, 18, pw, 18, 9, 9, "F");
  d.setGState(new (d as never as { GState: new (o: { opacity: number }) => unknown }).GState({ opacity: 1 }));
  d.setFillColor(...PAPER); d.roundedRect(W(d) - pw - 20, 18, pw, 18, 9, 9, "S");
  d.setTextColor(...PAPER); d.setFontSize(8); d.setFont("helvetica", "bold");
  d.text(pill, W(d) - pw / 2 - 20, 29.5, { align: "center" });
  // title
  d.setTextColor(...INK); d.setFontSize(20); d.setFont("helvetica", "bold");
  d.text(title, 20, 78);
  d.setFontSize(9); d.setFont("helvetica", "normal"); d.setTextColor(...MUTED);
  d.text(subtitle, 20, 88);
  // divider
  d.setDrawColor(...LINE); d.setLineWidth(0.4); d.line(20, 96, W(d) - 20, 96);
}
...
function pageFooter(d: Doc) {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    d.setFontSize(7); d.setFont("helvetica", "normal"); d.setTextColor(...MUTED);
    d.text("FinTrack AI · Confidential financial intelligence", 20, H(d) - 14);
    d.text(`Page ${i} of ${total}`, W(d) - 20, H(d) - 14, { align: "right" });
    // tiny brand dot
    d.setFillColor(...PRIMARY); d.circle(W(d) / 2, H(d) - 16, 1.2, "F");
  }
}

function sectionLabel(d: Doc, kicker: string, title: string, y: number) {
  d.setFontSize(7); d.setFont("helvetica", "bold"); d.setTextColor(...PRIMARY);
  d.text(kicker.toUpperCase(), 20, y);
  d.setFontSize(15); d.setFont("helvetica", "bold"); d.setTextColor(...INK);
  d.text(title, 20, y + 12);
  d.setDrawColor(...PRIMARY); d.setLineWidth(1.4); d.line(20, y + 16, 36, y + 16);
  return y + 26;
}

function kpiCard(d: Doc, x: number, y: number, w: number, h: number, label: string, value: string, sub: string, accent: RGB) {
  d.setFillColor(...PAPER); d.roundedRect(x, y, w, h, 8, 8, "F");
  d.setDrawColor(...LINE); d.setLineWidth(0.4); d.roundedRect(x, y, w, h, 8, 8, "S");
  // top accent
  d.setFillColor(...accent); d.roundedRect(x, y, w, 3, 1.5, 1.5, "F");
  d.setFontSize(7); d.setFont("helvetica", "bold"); d.setTextColor(...MUTED);
  d.text(label.toUpperCase(), x + 10, y + 16);
  d.setFontSize(15); d.setFont("helvetica", "bold"); d.setTextColor(...INK);
  d.text(value, x + 10, y + 32);
  if (sub) {
    d.setFontSize(7.5); d.setFont("helvetica", "normal"); d.setTextColor(...MUTED);
...
function kpiRow(d: Doc, y: number, items: { label: string; value: string; sub?: string; tone?: RGB }[]) {
  ...
  items.forEach((it, i) =>
    kpiCard(d, 20 + i * (cw + gap), y, cw, 50, it.label, it.value, it.sub || "", it.tone || PRIMARY));
  return y + 58;
}

function paragraph(d: Doc, x: number, y: number, w: number, text: string, opts: { size?: number; color?: RGB; bold?: boolean; lh?: number } = {}) {
...
function bullet(d: Doc, x: number, y: number, w: number, items: string[], tone: RGB = PRIMARY) {
  items.forEach((it) => {
    d.setFillColor(...tone); d.circle(x + 2, y - 2.5, 1.4, "F");
...
function areaChart(d: Doc, x: number, y: number, w: number, h: number, values: number[], labels: string[], color: RGB, label: string) {
  panel(d, x, y, w, h);
  ...
  d.setGState(new (d as never as { GState: new (o: { opacity: number }) => unknown }).GState({ opacity: 1 }));
  // line
  d.setDrawColor(...color); d.setLineWidth(1.3);
  for (let i = 0; i < pts.length - 1; i++) d.line(pts[i][0], pts[i][1], pts[i + 1][0], pts[i + 1][1]);
  pts.forEach(([xx, yy]) => { d.setFillColor(...color); d.circle(xx, yy, 1.2, "F"); });
  // legend
  d.setFillColor(...color); d.circle(x + 14, y + 10, 1.6, "F");
  d.setFontSize(7); d.setTextColor(...MUTED); d.text(label, x + 18, y + 12);
}

function multiLine(d: Doc, x: number, y: number, w: number, h: number, series: { color: RGB; values: number[]; label: string }[], labels: string[]) {
  panel(d, x, y, w, h);
  const { ix, iy, iw, ih } = gridFrame(d, x, y, w, h);
  ...
  drawAxes(d, ix, iy, iw, ih, max, min, labels);
  series.forEach((s) => {
    d.setDrawColor(...s.color); d.setLineWidth(1.4);
    ...
    s.values.forEach((v, i) => {
      ...
      const yy = iy + ih - ((v - min) / Math.max(1, max - min)) * ih;
      d.setFillColor(...s.color); d.circle(xx, yy, 1.3, "F");
    });
  });
  // legend
  let lx = x + 14;
  series.forEach((s) => {
    d.setFillColor(...s.color); d.circle(lx, y + 10, 1.6, "F");
    d.setFontSize(7); d.setTextColor(...MUTED); d.text(s.label, lx + 4, y + 12);
    lx += d.getTextWidth(s.label) + 16;
  });
}

function dualBars(d: Doc, x: number, y: number, w: number, h: number, data: { label: string; income: number; expense: number }[]) {
  panel(d, x, y, w, h);
  const { ix, iy, iw, ih } = gridFrame(d, x, y, w, h);
  const max = Math.max(1, ...data.flatMap((d) => [d.income, d.expense]));
  drawAxes(d, ix, iy, iw, ih, max, 0, data.map((d) => d.label));
  ...
  data.forEach((dd, i) => {
    ...
    d.setFillColor(...GOOD); d.roundedRect(cx - bw - 1.5, iy + ih - incH, bw, incH, 1.5, 1.5, "F");
    d.setFillColor(...BAD); d.roundedRect(cx + 1.5, iy + ih - expH, bw, expH, 1.5, 1.5, "F");
  });
  d.setFillColor(...GOOD); d.circle(x + 14, y + 10, 1.6, "F");
  d.setFontSize(7); d.setTextColor(...MUTED); d.text("Income", x + 18, y + 12);
  d.setFillColor(...BAD); d.circle(x + 52, y + 10, 1.6, "F");
...
function donut(d: Doc, cx: number, cy: number, r: number, data: { name: string; value: number }[]) {
  ...
  if (total === 0) {
    d.setDrawColor(...LINE); d.setLineWidth(10); d.circle(cx, cy, r, "S"); return;
  }
  let start = -Math.PI / 2;
  data.forEach((dd, i) => {
    ...
    d.setDrawColor(...col); d.setLineWidth(11);
  ...
  d.setFillColor(...PAPER); d.circle(cx, cy, r - 8, "F");
}
...
function scoreGauge(d: Doc, cx: number, cy: number, r: number, score: number, grade: string) {
  ...
  for (let i = 0; i < segs; i++) {
    ...
    d.line(cx + Math.cos(t1 - Math.PI / 2) * r, cy + Math.sin(t1 - Math.PI / 2) * r,
      cx + Math.cos(t2 - Math.PI / 2) * r, cy + Math.sin(t2 - Math.PI / 2) * r);
  }
  // value
  ...
  // text
  d.setTextColor(...INK); d.setFont("helvetica", "bold"); d.setFontSize(28);
  d.text(String(Math.round(score)), cx, cy + 4, { align: "center" });
  d.setFontSize(8); d.setFont("helvetica", "normal"); d.setTextColor(...MUTED);
...
function progressBar(d: Doc, x: number, y: number, w: number, ratio: number, tone: RGB, height = 5) {
  d.setFillColor(...LINE); d.roundedRect(x, y, w, height, height / 2, height / 2, "F");
  d.setFillColor(...tone); d.roundedRect(x, y, w * Math.max(0, Math.min(1, ratio)), height, height / 2, height / 2, "F");
}
...
function monthlyTrend(txs: ReportTx[], months: number) {
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    out.push({ name: d.toLocaleDateString("en", { month: "short" }), key, income: inc, expense: exp, savings: inc - exp });
  }
  return out;
}
...
function quarterlyTrend(txs: ReportTx[], quarters: number) {
  ...
  for (let i = quarters - 1; i >= 0; i--) {
    const end = new Date(); end.setMonth(end.getMonth() - i * 3);
...
function merchantStats(txs: ReportTx[]) {
  ...
  txs.filter((t) => t.type !== "income").forEach((t) => {
    ...
    const cur = map.get(k) || { name: k, count: 0, total: 0, lastDate: t.transaction_date, firstDate: t.transaction_date };
...
function pageCover(doc: Doc, input: ReportInput, score: number, grade: string) {
  ...
  // logo
  doc.setFillColor(...PAPER); doc.roundedRect(40, 60, 36, 36, 9, 9, "F");
  doc.setTextColor(...PRIMARY); doc.setFontSize(20); doc.setFont("helvetica", "bold");
  doc.text("FT", 58, 84, { align: "center" });
  doc.setTextColor(...PAPER); doc.setFontSize(12); doc.setFont("helvetica", "bold");
  doc.text("FinTrack AI", 84, 78);
  doc.setFontSize(8); doc.setFont("helvetica", "normal");
  doc.text("Personal Financial Operating System", 84, 87);

  // title
  doc.setFontSize(40); doc.setFont("helvetica", "bold"); doc.setTextColor(...PAPER);
  ...
  // meta card
  const cy = 330;
  doc.setFillColor(...PAPER); doc.roundedRect(40, cy, W(doc) - 80, 130, 12, 12, "F");
  ...
  metas.forEach((m, i) => {
    ...
    const x = 56 + col * half; const y = cy + 30 + row * 44;
  ...
  // health gauge card
  const by = 490;
  doc.setFillColor(...PAPER); doc.roundedRect(40, by, W(doc) - 80, 170, 12, 12, "F");
  doc.setFontSize(8); doc.setFont("helvetica", "bold"); doc.setTextColor(...MUTED);
  doc.text("FINANCIAL HEALTH SCORE", 60, by + 24);
  doc.setFontSize(11); doc.setFont("helvetica", "normal"); doc.setTextColor(...SUBINK);
  paragraph(doc, 60, by + 40, W(doc) / 2 - 20, "A composite of 8 factors — savings rate, budget adherence, goal progress, income & spending stability, emergency fund coverage, subscription health and debt ratio.", { size: 9, color: MUTED });
  scoreGauge(doc, W(doc) - 110, by + 90, 44, score, grade);

  // footer line
  doc.setTextColor(...PAPER); doc.setFontSize(8); doc.setFont("helvetica", "normal");
  doc.text("Confidential · Generated by Lumo AI · fintrack.ai", W(doc) / 2, H(doc) - 28, { align: "center" });
}

function pageHealthBreakdown(doc: Doc, input: ReportInput, hs: { score: number; grade: string; factors: FactorScore[]; monthlyChange: number; trend: { date: string; score: number }[]; recommendations: string[] }) {
  brandedHeader(doc, "Financial Health Breakdown", "How your score is calculated across 8 weighted factors.", input.tier);
  let y = 110;
  ...
  doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.setTextColor(...dCol);
  doc.text(`${delta >= 0 ? "▲" : "▼"} ${Math.abs(delta)} pts vs prior month`, tx, y + 60);
  doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...MUTED);
  paragraph(doc, tx, y + 74, W(doc) - tx - 40, "Each factor is independently scored 0–100, then combined using research-backed weights. Lower-scoring factors drive the recommendations below.", { size: 8, color: MUTED });
  // mini trend
  ...
  hs.factors.forEach((f) => {
    ...
    doc.setFontSize(10); doc.setFont("helvetica", "bold"); doc.setTextColor(...tone);
    doc.text(`${Math.round(f.score)}`, W(doc) - 32, fy, { align: "right" });
    progressBar(doc, 32, fy + 4, W(doc) - 80, f.score / 100, tone, 4);
    fy += 22;
  });
  y += 282;
...
function periodCompare(input: ReportInput) {
  ...
  const sum = (a: ReportTx[], pred: (t: ReportTx) => boolean) => a.filter(pred).reduce((s, t) => s + Number(t.amount), 0);
  ...
  return { curInc, curExp, curSav, prevInc, prevExp, prevSav, expDelta: curExp - prevExp, savDelta: curSav - prevSav, incDelta: curInc - prevInc };
}

function pageExecutiveSummary(doc: Doc, input: ReportInput, hs: { score: number; grade: string }, period: ReturnType<typeof periodCompare>) {
  brandedHeader(doc, "Executive Summary", "AI-written financial analysis based on your actual data.", input.tier);
  let y = 110;
  y = kpiRow(doc, y, [
    { label: "Income", value: fmt(input.stats.income, input.currency), sub: `${period.incDelta >= 0 ? "+" : "−"}${fmt(Math.abs(period.incDelta), input.currency)} vs prior`, tone: GOOD },
    { label: "Expenses", value: fmt(input.stats.expenses, input.currency), sub: `${period.expDelta >= 0 ? "+" : "−"}${fmt(Math.abs(period.expDelta), input.currency)} vs prior`, tone: BAD },
    { label: "Savings", value: fmt(input.stats.savings, input.currency), sub: `Rate ${pct(input.stats.savingsRate)}`, tone: input.stats.savings >= 0 ? GOOD : BAD },
    { label: "Health Score", value: `${hs.score}/100`, sub: hs.grade, tone: hs.score >= 70 ? GOOD : hs.score >= 50 ? WARN : BAD },
  ]);

  // AI narrative (300-500 words target)
  const narrative = buildExecutiveNarrative(input, hs, period);
  panel(doc, 20, y, W(doc) - 40, H(doc) - y - 40, "AI Executive Overview");
  paragraph(doc, 30, y + 28, W(doc) - 60, narrative, { size: 9.5, lh: 13, color: SUBINK });
}

function buildExecutiveNarrative(input: ReportInput, hs: { score: number; grade: string }, p: ReturnType<typeof periodCompare>) {
  ...
  const goalAvg = input.goals.length ? input.goals.reduce((s, g) => s + (Number(g.current_amount) / Math.max(1, Number(g.target_amount))) * 100, 0) / input.goals.length : 0;
  ...
  if (input.stats.savingsRate >= 20) {
    parts.push(`Your savings discipline is a clear strength — sustaining above 20% places you in the top quartile of users at your income tier and is the single most important driver of long-term net worth.`);
  } else if (input.stats.savingsRate >= 10) {
    parts.push(`Your savings rate is in the healthy-but-improvable band. Pushing toward 20% would compound meaningfully: each 5-point lift translates to roughly ${fmt(input.stats.income * 0.05, cur)} more saved per month at current income.`);
  } else {
    parts.push(`Savings rate is the weakest link this period and warrants direct attention. Even a small 5-point lift would free ${fmt(input.stats.income * 0.05, cur)} in monthly headroom for goals and emergency reserves.`);
  }

  if (top) {
    parts.push(`Spending was concentrated in ${top.category} (${fmt(top.current, cur)}, ${pct((top.current / Math.max(1, input.stats.expenses)) * 100)} of total).`);
    if (grew && grew.growth > 15) parts.push(`The fastest-rising category was ${grew.category}, up ${pct(grew.growth)} month-over-month — a clear signal to investigate before it compounds.`);
    if (cut && cut.growth < -10) parts.push(`On the upside, ${cut.category} dropped ${pct(Math.abs(cut.growth))}, showing intentional restraint worth replicating elsewhere.`);
  }

  if (input.budgets && input.budgets.length) {
    parts.push(`Across ${input.budgets.length} active budget${input.budgets.length > 1 ? "s" : ""} you used ${pct(budgetUse)} of the configured envelope${breached ? `, with ${breached} category${breached > 1 ? "ies" : "y"} breaching the cap` : ", staying inside every limit"}.`);
  } else {
    parts.push(`No category budgets are currently configured. Defining envelopes for your top three spending buckets typically improves adherence by 30–40% within two cycles.`);
  }

  if (goalsActive.length) {
    parts.push(`You are tracking ${input.goals.length} goal${input.goals.length > 1 ? "s" : ""} with an average progress of ${pct(goalAvg)}. At your current savings velocity, ${goalsActive.length} remain in active accumulation.`);
  } else if (input.goals.length === 0) {
    parts.push(`No savings goals are defined. A 3–6 month emergency fund (≈ ${fmt(input.stats.expenses * 3, cur)} to ${fmt(input.stats.expenses * 6, cur)}) is the highest-leverage first target.`);
  } else {
    parts.push(`Every defined goal has been reached — consider promoting a long-horizon investment goal next.`);
  }

  if (subs.length) {
    parts.push(`Subscription radar detected ${subs.length} recurring charge${subs.length > 1 ? "s" : ""} representing roughly ${fmt(subTotal, cur)} per month. Auditing the bottom-quartile services typically reclaims 10–20% of subscription spend without lifestyle impact.`);
  }
  ...
  parts.push(`Your composite Financial Health Score stands at ${hs.score}/100 (${hs.grade}). The following pages quantify each driver, model the next 30/90/365 days, and translate the analysis into a prioritized action plan.`);
...
function pageCashflow(doc: Doc, input: ReportInput) {
  ...
  areaChart(doc, 22, y + 18, W(doc) - 44, 118, monthly.map(m => m.savings), monthly.map(m => m.name), GOOD, "Savings");
  y += 152;

  // dual column: weekly net line + quarterly bars
  const hw = (W(doc) - 50) / 2;
  panel(doc, 20, y, hw, 130, "Weekly Net (8 weeks)");
  multiLine(doc, 22, y + 16, hw - 4, 110, [{ color: PRIMARY, label: "Net Cashflow", values: weekly.map(w => w.net) }], weekly.map(w => w.name));
  panel(doc, 30 + hw, y, hw, 130, "Quarterly Income vs Expense");
  dualBars(doc, 32 + hw, y + 16, hw - 4, 110, quarterly.map(q => ({ label: q.name, income: q.income, expense: q.expense })));
  y += 142;

  // yearly trend
  panel(doc, 20, y, W(doc) - 40, 110, "Yearly Performance");
  dualBars(doc, 22, y + 16, W(doc) - 44, 88, yearly.map(yr => ({ label: yr.name, income: yr.income, expense: yr.expense })));
  y += 122;
  ...
  paragraph(doc, 30, y + 26, W(doc) - 60,
...
function pageCategoryIntel(doc: Doc, input: ReportInput) {
  brandedHeader(doc, "Category Intelligence", "Where every unit of currency flows, with trends and budget variance.", input.tier);
  let y = 110;
  ...
  doc.setFontSize(6); doc.setTextColor(...MUTED); doc.text("TOTAL", 90, y + 116, { align: "center" });
  // legend
  let ly = y + 28;
  items.slice(0, 8).forEach((it, i) => {
    const col = CAT_HEX[it.name] || palette(i);
    doc.setFillColor(...col); doc.roundedRect(170, ly - 5, 8, 8, 1.5, 1.5, "F");
    doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.setTextColor(...INK);
    doc.text(it.name, 184, ly);
    doc.setFont("helvetica", "normal"); doc.setTextColor(...MUTED);
    doc.text(`${fmt(it.value, input.currency)} · ${pct((it.value / total) * 100)}`, W(doc) - 32, ly, { align: "right" });
    ly += 18;
  });
  y += 212;
  ...
  const cw = (W(doc) - 60) / 3;
  ([
    ["Top Category", top?.name || "—", top ? `${fmt(top.value, input.currency)} · ${pct((top.value / cur30Total) * 100)} of spend` : "", PRIMARY],
    ["Fastest Growing", fastest?.category || "—", fastest ? `▲ ${pct(fastest.growth)} · +${fmt(fastest.delta, input.currency)}` : "Stable", WARN],
    ["Most Reduced", reduced?.category || "—", reduced ? `▼ ${pct(Math.abs(reduced.growth))} · −${fmt(Math.abs(reduced.delta), input.currency)}` : "Stable", GOOD],
  ] as [string, string, string, RGB][]).forEach(([label, name, sub, c], i) => {
    const x = 20 + i * (cw + 10);
    panel(doc, x, y, cw, 76);
    doc.setFillColor(...c); doc.roundedRect(x, y, 3, 76, 1.5, 1.5, "F");
    doc.setFontSize(7); doc.setFont("helvetica", "bold"); doc.setTextColor(...MUTED);
    ...
    doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...c);
    doc.text(sub, x + 12, y + 56);
  });
  y += 86;

  // Budget variance table
  panel(doc, 20, y, W(doc) - 40, H(doc) - y - 40, "Budget Variance");
  const budgets = input.budgets || [];
  if (!budgets.length) {
    paragraph(doc, 30, y + 30, W(doc) - 60, "No category budgets configured. Define envelopes to unlock variance tracking and overrun alerts.", { color: MUTED });
  } else {
    let by = y + 32;
    budgets.slice(0, 6).forEach((b) => {
      ...
      doc.setTextColor(...tone); doc.setFont("helvetica", "bold");
      doc.text(`${variance >= 0 ? "+" : "−"}${fmt(Math.abs(variance), input.currency)}`, W(doc) - 32, by, { align: "right" });
      progressBar(doc, 32, by + 4, W(doc) - 64, Math.min(1.5, ratio), tone, 4);
      by += 22;
    });
  }
}
...
function pageGoalIntel(doc: Doc, input: ReportInput) {
  brandedHeader(doc, "Goal Intelligence", "Realistic ETAs, required contributions and completion probability.", input.tier);
  let y = 110;
  if (input.goals.length === 0) {
    panel(doc, 20, y, W(doc) - 40, 100, "No Goals Defined");
    paragraph(doc, 30, y + 36, W(doc) - 60, "Define an emergency fund (3–6 months of expenses) or a major savings goal to unlock pacing intelligence, ETA forecasts and completion probability scoring.", { color: MUTED });
    return;
  }
  ...
  input.goals.slice(0, 5).forEach((g) => {
    ...
    if (remaining === 0) etaLabel = "Achieved";
      ...
      if (monthsETA > 60) etaLabel = "60+ months at current pace";
        ...
        const eta = new Date(); eta.setMonth(eta.getMonth() + Math.ceil(monthsETA));
    ...
    doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...MUTED);
    doc.text(`${g.category || "Goal"} · target ${fmt(target, input.currency)}${g.deadline ? ` · deadline ${g.deadline}` : ""}`, 32, y + 32);
    // risk pill
    doc.setFillColor(...riskTone); doc.roundedRect(W(doc) - 70, y + 14, 50, 14, 7, 7, "F");
    doc.setTextColor(...PAPER); doc.setFontSize(7); doc.setFont("helvetica", "bold");
    doc.text(`${risk.toUpperCase()} RISK`, W(doc) - 45, y + 23, { align: "center" });
    // progress bar
    progressBar(doc, 32, y + 42, W(doc) - 64, ratio, ACCENT, 6);
    doc.setFontSize(8); doc.setTextColor(...SUBINK);
    doc.text(`${pct(ratio * 100)} complete · ${fmt(current, input.currency)} / ${fmt(target, input.currency)}`, 32, y + 58);
    // metrics row
    const mx = 32, my = y + 72;
    const colW = (W(doc) - 64) / 4;
    [
      ["ETA", etaLabel],
      ["Required / mo", requiredMonthly > 0 ? fmt(requiredMonthly, input.currency) : (sharePer > 0 ? fmt(sharePer, input.currency) : "—")],
...
function pageTransactionIntel(doc: Doc, input: ReportInput) {
  brandedHeader(doc, "Transaction Intelligence", "Top transactions, merchant trends, recurring patterns and subscriptions.", input.tier);
  let y = 110;
  ...
  // top stats row
  y = kpiRow(doc, y, [
    { label: "Most Frequent Merchant", value: mostFreq?.name || "—", sub: mostFreq ? `${mostFreq.count} transactions` : "", tone: SKY },
    { label: "Highest Spend Merchant", value: highest?.name || "—", sub: highest ? fmt(highest.total, input.currency) : "", tone: PINK },
    { label: "Active Subscriptions", value: String(subs.length), sub: `${fmt(subs.reduce((s, x) => s + x.amount, 0), input.currency)} / month`, tone: ACCENT },
  ...
  let sy = y + 28;
  if (!subs.length) {
    paragraph(doc, 30, sy + 4, cw - 20, "No recurring subscriptions detected from your transaction patterns.", { color: MUTED, size: 8 });
  } else {
  ...
  let my = y + 28;
  [...merchants].sort((a, b) => b.total - a.total).slice(0, 8).forEach((m) => {
    doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.setTextColor(...INK);
...
function pageForecast(doc: Doc, input: ReportInput) {
  ...
  // 30 / 90 / 12mo cards
  y = kpiRow(doc, y, [
    { label: "30-Day Spending", value: fmt(fc.nextMonthSpending.value, input.currency), sub: `Confidence ${Math.round(fc.nextMonthSpending.confidence * 100)}%`, tone: BAD },
    { label: "30-Day Savings", value: fmt(fc.nextMonthSavings.value, input.currency), sub: `Confidence ${Math.round(fc.nextMonthSavings.confidence * 100)}%`, tone: fc.nextMonthSavings.value >= 0 ? GOOD : BAD },
    { label: "90-Day Cash", value: fmt(fc.expectedCashBalance.value + avgSav * 2, input.currency), sub: "Projected balance", tone: PRIMARY },
    { label: "12-Month Savings", value: fmt(avgSav * 12, input.currency), sub: "At current pace", tone: TEAL },
  ]);

  // savings projection chart
  const projected = [...m6.map(m => m.savings), fc.nextMonthSavings.value, fc.nextMonthSavings.value * 1.02, fc.nextMonthSavings.value * 1.04];
  const labels = [...m6.map(m => m.name), "+1m", "+2m", "+3m"];
  panel(doc, 20, y, W(doc) - 40, 140, "Savings Projection (history → 90 days)");
  multiLine(doc, 22, y + 18, W(doc) - 44, 118, [
    { color: GOOD, label: "Historical", values: m6.map(m => m.savings).concat([NaN, NaN, NaN] as number[]).map(v => Number.isFinite(v) ? v : 0).slice(0, m6.length).concat([0, 0, 0]) },
    { color: PRIMARY, label: "Forecast", values: Array(m6.length - 1).fill(0).concat([m6[m6.length - 1]?.savings || 0, fc.nextMonthSavings.value, fc.nextMonthSavings.value * 1.02, fc.nextMonthSavings.value * 1.04]) },
  ], labels);
  y += 152;
  ...
  if (!fc.goalCompletions.length) {
    ...
    fc.goalCompletions.slice(0, 5).forEach((g) => {
      ...
      const etaTxt = !g.etaMonths ? "Boost savings to unlock"
        : g.etaMonths > 60 ? "60+ months at current pace"
        : g.etaDate ? `ETA ${new Date(g.etaDate).toLocaleDateString("en", { month: "short", year: "numeric" })}` : "—";
      doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.setTextColor(...INK);
      doc.text(g.name, 32, gy);
      doc.setFont("helvetica", "normal"); doc.setTextColor(...MUTED);
      doc.text(etaTxt, 32, gy + 10);
      doc.setFillColor(...tone); doc.roundedRect(W(doc) - 70, gy - 6, 50, 14, 7, 7, "F");
      doc.setTextColor(...PAPER); doc.setFontSize(7); doc.setFont("helvetica", "bold");
...
async function pageCoachIntel(doc: Doc, input: ReportInput) {
  brandedHeader(doc, "AI Coach Intelligence", "Topics, key insights and engagement metrics from your conversations with Lumo.", input.tier);
  let y = 110;
  ...
  if (input.userId) {
    const [{ data: h }, { data: a }] = await Promise.all([
      supabase.from("ai_history").select("message, ai_response, created_at, persona").eq("user_id", input.userId).order("created_at", { ascending: false }).limit(100),
      supabase.from("automation_logs").select("action_taken, rule_name, amount_saved, created_at").eq("user_id", input.userId).order("created_at", { ascending: false }).limit(50),
    ]);
  ...
  const avgLen = history.length ? Math.round(history.reduce((s, h) => s + (h.message?.length || 0), 0) / history.length) : 0;

  y = kpiRow(doc, y, [
    { label: "Total Conversations", value: String(history.length), sub: `${last30.length} in last 30d`, tone: PRIMARY },
    { label: "Personas Engaged", value: String(personasUsed.size || 0), sub: Array.from(personasUsed).slice(0, 2).join(", ") || "—", tone: ACCENT },
    { label: "Avg Question Length", value: `${avgLen} chars`, sub: avgLen > 80 ? "Detailed prompts" : "Concise prompts", tone: TEAL },
    { label: "Automations Run", value: String(autoLogs.length), sub: `${fmt(autoLogs.reduce((s, a) => s + (Number(a.amount_saved) || 0), 0), input.currency)} saved`, tone: GOOD },
  ]);
  ...
  if (!topTopics.length) {
    paragraph(doc, 30, y + 36, cw - 20, "No coach conversations recorded yet. Open Lumo and ask a financial question to seed your personalized advisor.", { color: MUTED, size: 8 });
  } else {
    ...
    topTopics.forEach(([topic, count], i) => {
      ...
      progressBar(doc, 30, ty + 4, cw - 20, count / max, tone, 4);
      ty += 18;
    });
  }
  ...
  const topQs = Array.from(qMap.values()).sort((a, b) => b.count - a.count).slice(0, 5);
  let qy = y + 32;
  if (!topQs.length) {
    paragraph(doc, 40 + cw, qy + 4, cw - 20, "No questions recorded yet.", { color: MUTED, size: 8 });
  } else {
    topQs.forEach((q) => {
      doc.setFillColor(...PRIMARY); doc.circle(40 + cw, qy - 2, 1.4, "F");
      const lines = doc.splitTextToSize(`"${q.question.slice(0, 90)}${q.question.length > 90 ? "…" : ""}"`, cw - 30);
      doc.setFontSize(8); doc.setFont("helvetica", "italic"); doc.setTextColor(...SUBINK);
  ...
  if (last30.length >= 5) insights.push(`Active engagement — ${last30.length} conversations in the last 30 days indicate strong financial intentionality.`);
  else if (history.length > 0) insights.push(`Engagement is light (${last30.length} conversations / 30d). Weekly check-ins compound coaching value.`);
  if (topTopics[0]) insights.push(`Primary focus area: ${topTopics[0][0]} (${topTopics[0][1]} mentions). Lumo will continue prioritizing this domain.`);
  if (topTopics.find(([t]) => t === "Saving" || t === "Goals") && input.stats.savingsRate < 15) insights.push(`You ask about saving and goals frequently, yet your savings rate is ${pct(input.stats.savingsRate)} — execution gap worth closing.`);
  if (autoLogs.length) insights.push(`${autoLogs.length} automation${autoLogs.length > 1 ? "s have" : " has"} fired this period, saving an aggregated ${fmt(autoLogs.reduce((s, a) => s + (Number(a.amount_saved) || 0), 0), input.currency)}.`);
  if (personasUsed.size > 1) insights.push(`You consult ${personasUsed.size} personas — diverse coaching styles correlate with better adherence.`);
  if (!insights.length) insights.push("Once you accumulate more conversation history, this section will surface engagement patterns and gap analyses.");
  bullet(doc, 30, y + 28, W(doc) - 60, insights.slice(0, 6), PRIMARY);
}

function pageActionPlan(doc: Doc, input: ReportInput, score: number) {
  brandedHeader(doc, "Financial Action Plan", "Prioritized roadmap for the next 90 days, derived from your data.", input.tier);
  let y = 110;
  const subs = detectSubs(input.transactions);
  const subTotal = subs.reduce((s, x) => s + x.amount, 0);
  ...
  if (subs.length) immediate.push(`Audit the ${subs.length} detected subscription${subs.length > 1 ? "s" : ""} (~${fmt(subTotal, input.currency)}/mo) — cancel anything unused in 30 days.`);
  if (breached.length) immediate.push(`Reset overspent budgets: ${breached.slice(0, 3).map(b => b.category).join(", ")}. Reduce next month's cap by 10% or raise it deliberately.`);
  if (top) immediate.push(`Set a one-month cap on ${top.name} at ${fmt(top.value * 0.85, input.currency)} (15% reduction from current).`);
  if (input.stats.savings > 0) immediate.push(`Automate a ${fmt(input.stats.savings * 0.2, input.currency)} transfer to savings within 7 days to lock in this month's progress.`);

  const thirty: string[] = [];
  if (input.stats.savingsRate < 20) thirty.push(`Lift savings rate from ${pct(input.stats.savingsRate)} toward ${pct(Math.max(20, input.stats.savingsRate + 5))} — target ${fmt(input.stats.income * 0.05, input.currency)} additional saved.`);
  if (input.goals.length === 0) thirty.push(`Open an Emergency Fund goal: target ${fmt(input.stats.expenses * 3, input.currency)} (3 months of expenses).`);
  else if (lagging) thirty.push(`Accelerate "${lagging.goal_name}": increase contribution by ${fmt(Math.max(500, Number(lagging.target_amount) * 0.02), input.currency)}/month.`);
  thirty.push("Enable automation rules for budget alerts on your top 3 spending categories.");
  if (input.transactions.filter(t => t.recurring).length === 0) thirty.push("Flag recurring expenses in the transactions log to power subscription radar.");

  const ninety: string[] = [];
  ninety.push(`Lift Financial Health Score from ${score} toward ${Math.min(100, score + 15)} (focus on the 3 lowest factors first).`);
  ...
  const sections: { title: string; tone: RGB; items: string[] }[] = [
    { title: "Immediate (this week)", tone: BAD, items: immediate.length ? immediate : ["No urgent actions detected — maintain current cadence."] },
  ...
  sections.forEach((s) => {
    ...
    let iy = y + 36;
    s.items.forEach((it) => {
      doc.setFillColor(...s.tone); doc.circle(36, iy - 2.5, 1.4, "F");
...
export async function exportReport(input: ReportInput): Promise<void> {
  ...
  doc.addPage(); pageHealthBreakdown(doc, input, hs);
  doc.addPage(); pageExecutiveSummary(doc, input, hs, period);
  doc.addPage(); pageCashflow(doc, input);
  doc.addPage(); pageCategoryIntel(doc, input);
  doc.addPage(); pageGoalIntel(doc, input);
  doc.addPage(); pageTransactionIntel(doc, input);
  doc.addPage(); pageForecast(doc, input);
  doc.addPage(); await pageCoachIntel(doc, input);
  doc.addPage(); pageActionPlan(doc, input, hs.score);
