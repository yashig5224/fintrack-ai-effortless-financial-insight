...
export default function PricingPreview() {
  ...
  const handleSelect = async (planId: string) => {
    ...
    if (plan.id === "free") { navigate(user ? "/app" : "/login?signup=1"); return; }
    const planKey = cycle === "yearly" ? plan.planKeyYearly : plan.planKeyMonthly;
    if (!planKey) return;
    if (!user) {
      setPendingCheckout({ planKey, planName: plan.name, cycle });
    ...
    await openRazorpayCheckout({
      ...
      onError: (e) => { toast.error(e.message); setLoading(null); },
    });
  };
    ...
    <section id="pricing" className="relative px-6 py-24 md:py-32">
      ...
      <div className="max-w-6xl mx-auto text-center mb-14">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <Sparkles className="w-3.5 h-3.5 text-indigo-500" /> Pricing
        </motion.div>
        ...
        <p className="mt-5 text-lg text-slate-600 max-w-2xl mx-auto">From everyday tracking to elite AI wealth forecasting. Cancel anytime.</p>
        ...
        <div className="mt-8 inline-flex items-center gap-1 p-1 rounded-full bg-white/80 backdrop-blur border border-slate-200 shadow-sm">
          {(["monthly", "yearly"] as BillingCycle[]).map((c) => (
            <button key={c} onClick={() => setCycle(c)}
              ...
              {cycle === c && (
                <motion.div layoutId="pricing-preview-pill"
        ...
        {PLANS.map((plan, i) => {
          ...
          return (
            <motion.div key={plan.id}
              ...
              whileHover={{ y: -6 }}
              className={`relative rounded-3xl bg-gradient-to-br ${plan.gradient} ${plan.ring} p-7 shadow-xl shadow-slate-900/5 ${plan.highlight ? "md:scale-[1.03] md:-translate-y-2" : ""} transition-shadow hover:shadow-2xl hover:shadow-indigo-500/10`}>
              {plan.highlight && (
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-semibold tracking-tight text-slate-900">₹{amount.toLocaleString("en-IN")}</span>
                {amount > 0 && (<span className="text-sm text-slate-500 ml-1">/{cycle === "yearly" ? "year" : "month"}</span>)}
              </div>

              <Button onClick={() => handleSelect(plan.id)} disabled={isLoading}
                className={`mt-5 w-full rounded-xl py-5 font-semibold shadow-lg transition-all ${plan.id === "elite" ? "bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-700 hover:to-fuchsia-700 text-white shadow-violet-500/30" : plan.id === "pro" ? "bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white shadow-indigo-500/30" : "bg-slate-900 hover:bg-slate-800 text-white shadow-slate-900/20"} hover:scale-[1.02]`}>
                {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>{plan.cta}<ArrowRight className="w-4 h-4 ml-1.5" /></>}
              </Button>
