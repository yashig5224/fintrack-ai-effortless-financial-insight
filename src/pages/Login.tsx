...
const LumoMascot = ({ isSignUp, submitting }: { isSignUp: boolean; submitting: boolean }) => (
  <motion.div
    animate={{ y: [0, -8, 0] }}
  ...
  >
    ...
    <motion.div
      ...
      className="absolute inset-0 rounded-full bg-gradient-to-tr from-sky-300/60 via-violet-300/60 to-emerald-200/60 blur-2xl"
    />
    {/* body */}
    <div className="relative w-full h-full rounded-[40%] bg-gradient-to-b from-white to-sky-50 border border-white shadow-[0_25px_80px_-20px_rgba(59,130,246,0.35)] flex items-center justify-center">
      ...
      {/* mouth — shifts with mode */}
      <motion.div
        key={isSignUp ? "smile" : "calm"}
        initial={{ scaleX: 0.6, opacity: 0 }}
        animate={{ scaleX: 1, opacity: 1 }}
        className={`absolute bottom-10 h-2 rounded-full bg-slate-900 ${
          isSignUp ? "w-8" : "w-5"
        }`}
        style={{ borderRadius: isSignUp ? "0 0 999px 999px" : "999px" }}
      />
      {/* status dot */}
